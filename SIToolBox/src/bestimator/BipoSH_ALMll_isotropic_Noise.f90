...
! Date    : 06.09.2017 (UW Madison) (Integration with BipoSH estimation
! complete). Final run before publishing. 
...
subroutine isotropicnoise(llmax,LMAX,clbllMAX,clbLMAX,nside,samplenumber,fileinput,fileclebsch,outfilepath)
   ...
   character :: fileinput*500,filename*500,filenamei*500,fileclebsch*500
   character :: outfilepath*500
   ...
   do i=0,MyLMax
     do j=0,i
       do kk=0,i
         ...
         write(cj,"(I1)")j
         write(cl1,"(I1)")kk
         filename  = trim(outfilepath)//'/AR_'//trim(ci)//trim(cj)//'_ll'//trim(cl1)//'.d'
         filenamei = trim(outfilepath)//'/AI_'//trim(ci)//trim(cj)//'_ll'//trim(cl1)//'.d'
   ...
   do samplenumber=0,samplenumber !5000
...
subroutine gauss_seidel(ALMll,ALMlli,bMap,bmapi,SMAP,SMAPi,RSMAP,RSMAPi,llmax,LMAX,Clebs)
   ...
   integer :: i,j,Nmax,k
   integer :: il,im,jl,jm,recno,recno1
...
!           if(i.ne.j) then
            ...
            call Sii(0,0,jl,jl,abs(jm),llmax,recno)
            Sum1 = Sum1 + (bmap(j)*Smat - bmapi(j)*Smati)/(ALMll(0,0,jl,jl)*Clebs(recno)*int((-1)**jm))
            Sum1i = Sum1i + (bmap(j)*Smati + bmapi(j)*Smat)/(ALMll(0,0,jl,jl)*Clebs(recno)*int((-1)**jm))
!           end if
          end do
       end do
       ...
       call n2lm(i,il,im)
       call Sii(0,0,il,il,im,llmax,recno)
       Sum1  = Sum1/(ALMll(0,0,il,il)*Clebs(recno)*int((-1)**im))   
       Sum1i = Sum1i/(ALMll(0,0,il,il)*Clebs(recno)*int((-1)**im))    !! Need to check
