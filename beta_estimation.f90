...
integer :: maxsample
real(dp), allocatable,dimension(:,:,:,:) :: ALMll
real(dp) :: approx
...
integer:: myid,numprocs,ierr,pprocs,rc
call mpi_init(ierr)
...
allocate(ALMll(0:LMAX,0:LMAX,0:llMAX,0:llMAX))
...
approx = 0.001    ! Its an importent parameter. If C_l for some particular l
                  ! becomes lesser then approx then C_l will be fixed to approx
 
call generate_derivative(llMax,LMAX,maxsample,approx,nside,fileinput,filefs,fileclebsch,ALMll)
...
subroutine generate_derivative(llmax,LMAX,maxsample,approx,nside,fileinput,filefs,fileclebsch,ALMll)
   ...
   open(unit=9169,file='output/beta_r0.d')
   open(unit=9170,file='output/beta_r1.d')
   open(unit=9171,file='output/beta.d')
   open(unit=9172,file='output/beta_i1.d')
...
*(ALMlli(1,j,l1,l1+1)-ALMlli_old(1,j,l1,l1+1))/ALMll(0,0,l1,l1)/ALMll(0,0,l2,l2)/2.0)*fs(l1)
      ...
      do j=0,1
         pbeta(j)  = pbeta(j)  - pbetadot(j)*epsilon1*(1-2*theta)
         pbetai(j) = pbetai(j) - pbetaidot(j)*epsilon1*(1-2*theta)
      end do
...
*(ALMlli(1,j,l1,l1+1)-ALMlli_old(1,j,l1,l1+1))/ALMll(0,0,l1,l1)/ALMll(0,0,l2,l2)/2.0)*fs(l1)
      ...
      do l1=20,llmax
       if(abs(ALMll(0,0,l1,l1)) .lt. approx) then
           write(*,*)'** APPRIXIMATION USED abs(ALMll(0,0,l1,l1)) < ',approx
           ALMll(0,0,l1,l1) = approx*ALMll(0,0,l1,l1)/abs(ALMll(0,0,l1,l1))
       end if
      end do
...
!stop
   ...
   !close(unit=9169) ! file = A00ll.d
   !close(unit=9170) ! file = A10ll.d
   close(unit=9171) ! file = A10ll1.d
   !close(unit=9172) ! file = A11l.d
