...
!   write(*,*)'Hi 1'
   ...
   open(1,file='../clebsch/clebs.dat',access='direct',recl=64, action='read',status="OLD")
      do i=0,LMAX
        do k=0,llmax
          l1=k           ! k --> l1  
          l2min=l1
...
!$omp end parallel do
      ...
      call calculateALM(Smapr,Smapi,lmax,llmax,PALMlldot,PALMllidot,Clebs)
        ...
        do l1=2,llmax-2              
          ...
          l2 = l1+1
          do j=0,1
  pbetadot(j) = pbetadot(j) + (-((-1.0)**j)*PALMlldot(1,j,l1,l1+1)+ &
sqrt((2.0*l1+3.0)*(2.0*l1+1.0))*int((-1)**(l1+l2)) &
*ALMll(1,j,l1,l1+1)/ALMll(0,0,l1,l1)/ALMll(0,0,l2,l2)/2.0)*fs(l1)
  pbetaidot(j) = pbetaidot(j) + (-((-1.0)**j)*PALMllidot(1,j,l1,l1+1)+ &
sqrt((2.0*l1+3.0)*(2.0*l1+1.0))*int((-1)**(l1+l2)) &
...
!$omp end parallel do
      ...
      do l1=2,llmax-1
        do j=0,1
          ALMll(1,j,l1,l1+1)   = beta(j)*fs(l1)
          ALMll(1,j,l1i+1,l1)   = beta(j)*fs(l1)
          ALMlli(1,j,l1,l1+1)  = betai(j)*fs(l1)
          ALMlli(1,j,l1+1,l1)  = betai(j)*fs(l1)
        end do
      end do
...
write(*,*)'Hi 5'
      ...
      do l1=2,llmax-2   
         ...
         do j=0,1
            l2 = l1+1
   pbetadot(j) = pbetadot(j) + (-((-1.0)**j)*PALMlldot(1,j,l1,l1+1)+ &
sqrt((2.0*l1+1.0)*(2.0*l1+3.0))*int((-1)**(l1+l2)) &
*ALMll(1,j,l1,l1+1)/ALMll(0,0,l1,l1)/ALMll(0,0,l2,l2)/2.0)*fs(l1)
   pbetaidot(j)= pbetaidot(j)+ (-((-1.0)**j)*PALMllidot(1,j,l1,l1+1)+ &
sqrt((2.0*l1+1.0)*(2.0*l1+3.0))*int((-1)**(l1+l2)) &
*ALMlli(1,j,l1,l1+1)/ALMll(0,0,l1,l1)/ALMll(0,0,l2,l2)/2.0)*fs(l1)
...
!$omp end parallel do
      ...
      do j=0,1
         pbeta(j)  = pbeta(j)  - pbetadot(j)*epsilon1*(1-2.0*theta)
         pbetai(j) = pbetai(j) - pbetaidot(j)*epsilon1*(1-2.0*theta)
      end do
...
!$omp end parallel do
           ...
           ALMll(i,j,h,k) = almllr
           ALMli(i,j,h,k) = almlli
         end do
       end do  
     end do
   end do
